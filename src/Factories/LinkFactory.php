<?php
namespace Lagdo\Polr\Api\Factories;
...
class LinkFactory {
    ...
    private static function formatLink($link_ending, $secret_ending = false)
    {
        ...
        * @return string
        */
        $short_url = (env('RETURN_ENDING')) ? $link_ending :
            env('APP_PROTOCOL') . env('APP_ADDRESS') . '/' . $link_ending;
    ...
    public static function createLink($long_url, $is_secret = false, $custom_ending = null,
        $link_ip = '127.0.0.1', $creator = false, $return_object = false, $is_api = false)
    {
        /**
        ...
        */
        ...
        if(strlen($long_url) > self::MAXIMUM_LINK_LENGTH)
        {
            ...
            // throw an Exception
            throw new \Exception('Sorry, but your link is longer than the maximum length allowed.');
        }
        ...
        if($is_already_short)
        {
            throw new \Exception('Sorry, but your link already looks like a shortened URL.');
        }

        if(!$is_secret && (!isset($custom_ending) || $custom_ending === '') &&
